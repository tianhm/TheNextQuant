...
class MyStrategy:
    ...
    def initialize(self):
        """ 初始化
        """
        self.order_manager = OrderManager(self.platfrom, self.account, self.access_key, self.secret_key, self.symbol,
                                          self.name)
    ...
    async def create_order(self):
        ...
        price = "0.031898"
        quantity = "0.2"
        order_no = await self.order_manager.create_order(action, price, quantity)
